...
function createObject(x, y, mass) {
    this.x = x;
    this.y = y;
    this.vx = random(-10, 10) / mass / 2;
    this.vy = random(-10, 10) / mass / 2;
    this.number = number++;
    this.mass = mass * 1;
}
...
function newNode() {
    var Object = new createObject(random(0, w - 400) + 200, random(0, h - 400) + 200, 1); //var Object = new createObject(random(0, w), random(0, h));
    nodes.push(Object);
}
...
function drawNodes(item, index) {
    ...
    var consts = 25;
    var consts2 = 4;
    fill((nodes[index].mass * consts) / 2 + 255 / 2, (nodes[index].mass * consts) / 2 + 255 / 2, 0);
    ellipse(nodes[index].x, nodes[index].y, nodes[index].mass * consts2 + 2, nodes[index].mass * consts2 + 2);
}
...
function gravity(item, index) {
    for (var i = 0; i < nodes.length; i++) {
        if (index != i) {
            ...
            if (distance < 1000) {
                var force = (.1 * (nodes[i].mass + nodes[index].mass) / distance) * nodes[i].mass; // Get the force from the (c*(m1+m2)/r)/mass instead of (G(m1+m2)/r^2)/mass
...
function checks(item, index) {
    ...
    if (nodes.length > maxNumber) nodes.splice(nodes.length - 1, 1);
}
